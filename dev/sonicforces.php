<?php
	...
	$sql = "SELECT v.username, t.team FROM team_minigame t INNER JOIN viewers v ON v.user_id=t.user_id WHERE t.event='Sonic'";
	...
	foreach($res AS $line) {
	    if($line['team'] == 'e') {
	        $eggmanMembers[] = $line["username"];
	    } else {
	        $sonicMembers[] = $line['username'];
	    }
	}
	$tbl_inner = '';
	for($i=0; $i<max(count($eggmanMembers), count($sonicMembers)); $i++) {
	    $tbl_inner .= "<tr><td>";
	    $tbl_inner .= $sonicMembers[$i];
	    $tbl_inner .= "</td><td>";
	    $tbl_inner .= $eggmanMembers[$i];
	    $tbl_inner .= "</td></tr>\n";
	}
	$disp_txt = "Type !join Sonic or !join Eggman in YouTube chat to pick a side!";
?>
...
<div class="bannerh2"><p style="text-align: center;"><img src="/images/sanicspeeed.jpg" alt="Sonic Forces Teams" style="width:60%"></p></div> <!--"width:700px;height:394px;"-->
	<div class="table"><table width="60%" align="center">
		<tbody>
			<tr> <th>SONIC</th> <th>EGGMAN</th> </tr>
